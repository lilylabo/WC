...
<?php if( usces_is_membersystem_state() ) : ?>
		<tr><th scope="row" colspan="2"><?php if( $member_regmode == 'editmemberfromcart' ) : ?><span class="em"><?php _e('*', 'usces'); ?></span><?php endif; ?><?php _e('password', 'usces'); ?></th></tr>
		<tr><td colspan="2"><input name="customer[password1]" style="width:100px" type="password" value="<?php echo esc_attr($usces_entries['customer']['password1']); ?>"<?php wcmb_set_istyle( WCMB_ISTYLE_ALP ) ?> /><?php if( $member_regmode != 'editmemberfromcart' ) echo '<br />'.__('When you enroll newly, please fill it out.', 'usces'); ?></td></tr>
		<tr><th scope="row" colspan="2"><?php if( $member_regmode == 'editmemberfromcart' ) : ?><span class="em"><?php _e('*', 'usces'); ?></span><?php endif; ?><?php _e('Password (confirm)', 'usces'); ?></th></tr>
		<tr><td colspan="2"><input name="customer[password2]" style="width:100px" type="password" value="<?php echo esc_attr($usces_entries['customer']['password2']); ?>"<?php wcmb_set_istyle( WCMB_ISTYLE_ALP ) ?> /><?php if( $member_regmode != 'editmemberfromcart' ) echo '<br />'.__('When you enroll newly, please fill it out.', 'usces'); ?></td></tr>
<?php endif; ?>
