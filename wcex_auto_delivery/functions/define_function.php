<?php
function wcad_define_functions() {
...
/**********************************************************************/
	...
	define('USCES_COL_ITEM_CHARGING_TYPE', 20);
	define('USCES_COL_REGULAR_UNIT', 21);
	define('USCES_COL_REGULAR_INTERVAL', 22);
	define('USCES_COL_REGULAR_FREQUENCY', 23);
	define('USCES_COL_ITEM_SHIPPING', 24);
	define('USCES_COL_ITEM_DELIVERYMETHOD', 25);
	define('USCES_COL_ITEM_SHIPPINGCHARGE', 26);
	define('USCES_COL_ITEM_INDIVIDUALSCHARGE', 27);

	define('USCES_COL_CATEGORY', 28);
	define('USCES_COL_POST_TAG', 29);
	define('USCES_COL_CUSTOM_FIELD', 30);

	define('USCES_COL_SKU_CODE', 31);
	define('USCES_COL_SKU_NAME', 32);
	define('USCES_COL_SKU_CPRICE', 33);
	define('USCES_COL_SKU_PRICE', 34);
	define('USCES_COL_SKU_RPRICE', 35);
	define('USCES_COL_SKU_ZAIKONUM', 36);
	define('USCES_COL_SKU_ZAIKO', 37);
	define('USCES_COL_SKU_UNIT', 38);
	define('USCES_COL_SKU_GPTEKIYO', 39);
			...
			switch( $key ) {
				...
				case USCES_COL_ITEM_CHARGING_TYPE:
					if( $data != 0 and $data != 2 ) {
						$mes = "No." . ($rows_num+1) . "\t".__('A value of the charging type is abnormal.', 'autodelivery');
						$logtemp .= $mes.$yn;
						$mestemp .= $mes.$br.$yn;
					}
					break;
				case USCES_COL_REGULAR_UNIT:
					if( $datas[USCES_COL_ITEM_CHARGING_TYPE] == 2 ) {
						if( $data != 'day' and $data != 'month' ) {
							$mes = "No." . ($rows_num+1) . "\t".__('A value of the unit purchase interval is abnormal.', 'autodelivery');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
					break;
				case USCES_COL_REGULAR_INTERVAL:
					if( $datas[USCES_COL_ITEM_CHARGING_TYPE] == 2 ) {
						if( !preg_match("/^[0-9]+$/", $data) || 1 >= $data ) {
							$mes = "No." . ($rows_num+1) . "\t".__('A value of the purchase interval is abnormal.', 'autodelivery');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
					break;
				case USCES_COL_REGULAR_FREQUENCY:
					if( $datas[USCES_COL_ITEM_CHARGING_TYPE] == 2 ) {
						if( !preg_match("/^[0-9]+$/", $data) && 0 != strlen($data) ) {
							$mes = "No." . ($rows_num+1) . "\t".__('A value of the purchase frequency is abnormal.', 'autodelivery');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
...
//usces_log('mysql2date : '.mysql2date('U', $cdatas['post_modified'], false) . '/' . mysql2date('U', current_time( 'mysql' ), false), 'acting_transaction.log');
			...
			$valstr .= '(' . $post_id . ", '_wcad_regular_unit','" . $datas[USCES_COL_REGULAR_UNIT] . "'),";
			$valstr .= '(' . $post_id . ", '_wcad_regular_interval','" . $datas[USCES_COL_REGULAR_INTERVAL] . "'),";
			if( isset($datas[USCES_COL_REGULAR_FREQUENCY]) ) $valstr .= '(' . $post_id . ", '_wcad_regular_frequency','" . $datas[USCES_COL_REGULAR_FREQUENCY] . "'),";
...
//			wp_set_object_terms($post_id, (array)$tags, 'post_tag');
			foreach( (array)$tags as $tag ) {
				...
				$query = $wpdb->prepare("SELECT COUNT(*) FROM $wpdb->term_relationships WHERE term_taxonomy_id = %d", 
					$term_taxonomy_id
				);
				$tct = $wpdb->get_var( $query );

				$query = $wpdb->prepare("UPDATE $wpdb->term_taxonomy SET count = %d WHERE term_taxonomy_id = %d", 
					$tct, $term_taxonomy_id
				);
				$dbres = $wpdb->query($query);
			}
...
//					}
		...
		$skurprice = ( !empty($datas[USCES_COL_SKU_RPRICE]) ) ? array( 'rprice' => $datas[USCES_COL_SKU_RPRICE] ) : array();
		$skuvalue['advance'] = serialize( $skurprice );
...
function usces_download_item_list() {
	...
	if( $usces_opt_item['chk_header'] == 1 ) {
		...
		$line .= $th_h.__('Unit', 'autodelivery').$th_f;
		$line .= $th_h.__('Interval', 'autodelivery').$th_f;
		$line .= $th_h.__('Frequency', 'autodelivery').$th_f;
		...
		$line .= $th_h.__('regular purchase price','autodelivery').$th_f;
	...
	foreach( (array)$rows as $array ) {
		...
		$line_item .= $td_h.$regular_unit.$td_f;
		$line_item .= $td_h.$regular_interval.$td_f;
		$line_item .= $td_h.$regular_frequency.$td_f;
		...
		foreach( $sku_meta as $sku_value ) {
			...
			$line_sku .= $td_h.usces_crform($rprice, false, false, 'return', false).$td_f;
