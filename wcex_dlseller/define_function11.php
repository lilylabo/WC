...
/**********************************************************************/
	...
	define('USCES_COL_SKU_CODE', 39);	//21
	define('USCES_COL_SKU_NAME', 40);	//22
	define('USCES_COL_SKU_CPRICE', 41);	//23
	define('USCES_COL_SKU_PRICE', 42);	//24
	define('USCES_COL_SKU_ZAIKONUM', 43);	//25
	define('USCES_COL_SKU_ZAIKO', 44);	//26
	define('USCES_COL_SKU_UNIT', 45);	//27
	define('USCES_COL_SKU_GPTEKIYO', 46);	//28
			...
			switch($key){
				...
				case USCES_COL_ITEM_DIVISION:
					...
					if( !in_array($data, $array_division) || '' == $data ) {
						$mes = "No." . ($rows_num+1) . "\t".__('A value of the division is abnormal.', 'dlseller');
						$logtemp .= $mes.$yn;
						$mestemp .= $mes.$br.$yn;
					}
					break;
				case USCES_COL_ITEM_CHARGING_TYPE:
					if( $data != 0 and $data != 1 ) {
						$mes = "No." . ($rows_num+1) . "\t".__('The value of the Charging Type is abnormal.', 'dlseller');
						$logtemp .= $mes.$yn;
						$mestemp .= $mes.$br.$yn;
					}
					break;
				...
				case USCES_COL_ITEM_FREQUENCY:
					if($datas[USCES_COL_ITEM_CHARGING_TYPE] == 1) {//継続課金のとき
						if( $data != 1 and $data != 6 and $data != 12 ) {
							$mes = "No." . ($rows_num+1) . "\t".__('The value of the Charging Interval is abnormal.', 'dlseller');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
				...
				case USCES_COL_ITEM_CHARGINGDAY:
					if($datas[USCES_COL_ITEM_CHARGING_TYPE] == 1) {//継続課金のとき
						if( $data < 1 or $data > 28 ) {
							$mes = "No." . ($rows_num+1) . "\t".__('The value of the Charging Date is abnormal.', 'dlseller');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
				...
				case USCES_COL_DLSELLER_INTERVAL:
					if( 0 < strlen($data) and !preg_match("/^[0-9]+$/", $data) ) {
						$mes = "No." . ($rows_num+1) . "\t".__('A value of the period is abnormal.', 'dlseller');
						$logtemp .= $mes.$yn;
						$mestemp .= $mes.$br.$yn;
					}
					break;
				case USCES_COL_DLSELLER_VALIDITY:
					if( 0 < strlen($data) and !preg_match("/^[0-9]+$/", $data) ) {
						$mes = "No." . ($rows_num+1) . "\t".__('A value of the validity is abnormal.', 'dlseller');
						$logtemp .= $mes.$yn;
						$mestemp .= $mes.$br.$yn;
					}
					break;
				case USCES_COL_DLSELLER_FILE:
					if($datas[USCES_COL_ITEM_DIVISION] == 'data') {
						if( 0 == strlen($data) ) {
							$mes = "No." . ($rows_num+1) . "\t".__('A file name is non-input.', 'dlseller');
							$logtemp .= $mes.$yn;
							$mestemp .= $mes.$br.$yn;
						}
					}
