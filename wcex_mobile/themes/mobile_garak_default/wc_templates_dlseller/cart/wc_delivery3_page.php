...
<?php if( dlseller_have_dlseller_content() ) : ?>
	<table class="customer_form">
		<tr><th scope="row"><span class="em"><?php _e('*', 'usces'); ?></span><?php _e('Terms of Use', 'dlseller'); ?></th></tr>
		<tr><td><div class="dlseller_terms"><?php dlseller_terms(); ?></div></td></tr>
		<tr><td><label for="terms"><input type="checkbox" name="offer[terms]" id="terms" /><?php _e('Agree', 'dlseller'); ?></label></td></tr>
	</table>
<?php endif; ?>
...
<?php $entry_order_note = empty($usces_entries['order']['note']) ? apply_filters('usces_filter_default_order_note', NULL) : $usces_entries['order']['note']; ?>
	<table class="customer_form" id="notes_table">
		...
		<tr><td><textarea name="offer[note]" id="note" class="notes"><?php echo esc_html($entry_order_note); ?></textarea></td></tr>
	</table>
